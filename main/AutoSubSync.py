...
if platform in ['Darwin', 'Linux']:
    ...
    for exe in executables:
        exe_path = os.path.join(ffmpeg_bin, exe) if exe in ["ffmpeg", "ffprobe"] else os.path.join(ffsubsync_bin, exe)
        if os.path.exists(exe_path):
            ...
            if not (current_permissions & stat.S_IEXEC):
                try:
                    ...
                    errors.append(f"Failed to set execute permissions for {exe_path}: {e}")
    if errors:
...
class ToolTip:
    ...
    def show_tooltip(self, event=None):
        if self.tooltip is None:
            ...
            label = tk.Label(self.tooltip, text=self.text, justify=tk.LEFT, wraplength=wraplength, background="#ffffe0", foreground="black", relief=tk.SOLID, borderwidth=1)
            # removed font=("tahoma", "8", "normal") from label
            label.pack(ipadx=1)
...
check_video_for_subtitle_stream_var = tk.BooleanVar(value=check_video_for_subtitle_stream_in_alass)
            ...
            for sub in subtitles:
                ...
                if subtitle_file.lower().endswith(tuple(unsupported_extensions)):
                    ...
                    if subtitle_file_converted:
                        ...
                        return False
                log_window.insert(tk.END, SYNCING_LOG_TEXT.format(completed_items + 1, total_items, os.path.basename(original_subtitle_file), os.path.basename(video_file)))
                if sync_tool == SYNC_TOOL_ALASS:
...
def process_files(filepaths, reference_pairs=False):
    ...
    if reference_pairs:
        ...
        messages = []
        if pairs_added > 0:
            messages.append(ADDED_PAIRS_MSG.format(pairs_added, 's' if pairs_added != 1 else ''))
        if duplicates > 0:
...
def reference_subtitle_subtitle_pairs():
    ...
    explanation_label = ttk.Label(explanation_frame, text=EXPLANATION_TEXT_IN_REFERENCE_SUBTITLE_PAIRING.format(program_name=PROGRAM_NAME), wraplength=800, justify="left", background=COLOR_BACKGROUND, foreground=COLOR_BW)
    ...
    ref_add_btn = TkButton(ref_header, text=BUTTON_ADD_FILES, font=f'Arial {font_size} bold', command=lambda: load_files(listbox_left, ref_file_paths, type="reference"), padx=4, pady=0, fg=COLOR_WB, bg=DEFAULT_BUTTON_COLOR, activeforeground=COLOR_WB, activebackground=DEFAULT_BUTTON_COLOR_ACTIVE, relief=tk.RIDGE, borderwidth=1, cursor="hand2",highlightthickness=0,takefocus=0,state='normal')
    ref_add_btn.grid(row=0, column=1, padx=(5, 0))
    ref_remove_btn = TkButton(ref_header, text=CONTEXT_MENU_REMOVE, font=f'Arial {font_size} bold', command=lambda: remove_selected_item(listbox_left, ref_file_paths), padx=4, pady=0, fg=COLOR_WB, bg=DEFAULT_BUTTON_COLOR, activeforeground=COLOR_WB, activebackground=DEFAULT_BUTTON_COLOR_ACTIVE, relief=tk.RIDGE, borderwidth=1, cursor="hand2",highlightthickness=0,takefocus=0,state='normal')
    ...
    sub_add_btn = TkButton(sub_header, text=BUTTON_ADD_FILES, font=f'Arial {font_size} bold', command=lambda: load_files(listbox_right, sub_file_paths, type = "subtitle"), padx=4, pady=0, fg=COLOR_WB, bg=DEFAULT_BUTTON_COLOR, activeforeground=COLOR_WB, activebackground=DEFAULT_BUTTON_COLOR_ACTIVE, relief=tk.RIDGE, borderwidth=1, cursor="hand2",highlightthickness=0,takefocus=0,state='normal')
    sub_add_btn.grid(row=0, column=1, padx=(5, 0))
    sub_remove_btn = TkButton(sub_header, text=CONTEXT_MENU_REMOVE, font=f'Arial {font_size} bold', command=lambda: remove_selected_item(listbox_right, sub_file_paths), padx=4, pady=0, fg=COLOR_WB, bg=DEFAULT_BUTTON_COLOR, activeforeground=COLOR_WB, activebackground=DEFAULT_BUTTON_COLOR_ACTIVE, relief=tk.RIDGE, borderwidth=1, cursor="hand2",highlightthickness=0,takefocus=0,state='normal')
    ...
    def cancel():
        window.destroy()
    cancel_btn = TkButton(frame_bottom, text=CANCEL_TEXT, command=cancel, padx=30, pady=10, fg=COLOR_WB, bg=DEFAULT_BUTTON_COLOR, activebackground=DEFAULT_BUTTON_COLOR_ACTIVE, activeforeground=COLOR_WB, relief=tk.RAISED, borderwidth=2, cursor="hand2",highlightthickness=0,takefocus=0,state='normal')
    cancel_btn.pack(side="left", padx=(0, 5))
    process_btn = TkButton(frame_bottom, text=PROCESS_PAIRS, command=process_pairs, padx=10, pady=10, fg=COLOR_WB, bg=BUTTON_COLOR_BATCH, activebackground=BUTTON_COLOR_BATCH_ACTIVE, activeforeground=COLOR_WB, relief=tk.RAISED, borderwidth=2, cursor="hand2",highlightthickness=0,takefocus=0,state='normal')
...
if platform == "Darwin":
    batch_input.bind("<Button-2>", lambda event: options_menu.post(event.x_root, event.y_root))
else:
