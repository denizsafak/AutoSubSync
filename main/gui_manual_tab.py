...
def setup_manual_sync_tab(self):
    ...
    if not hasattr(self, 'session_shifted_files'):
        self.session_shifted_files = {}  # Dictionary to track {file_path: total_shifted_ms}
    ...
    self.add_ms_prefix_checkbox.setChecked(self.config.get("add_ms_prefix_to_filename", DEFAULT_OPTIONS["add_ms_prefix_to_filename"]))
    ...
    self.manual_message_label.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Minimum)
    self.manual_message_label.setVisible(False)
    self.manual_message_label.setStyleSheet(f"QLabel {{ padding: 10px; border-radius: 5px; }}")
...
def validate_manual_sync_inputs(self):
    ...
    save_location = self.config.get("manual_save_location", DEFAULT_OPTIONS["manual_save_location"])
    
    # Validate destination folder if needed
    if save_location == "select_destination_folder":
        folder = self.config.get("manual_save_folder", "")
        if not folder:
            self._show_message("Please select a destination folder in the save location dropdown.", "error")
            return False
        if not os.path.exists(folder):
            self._show_message(f"The selected destination folder does not exist:\n{folder}", "error")
            return False
    
    # Use the new output path determination function
    add_prefix = self.add_ms_prefix_checkbox.isChecked()
    output_file = determine_manual_output_path(self, self.manual_input_box.file_path, shift_ms, add_prefix)
...
def _show_message(self, message, msg_type="info"):
    ...
    if msg_type == "error":
        self.manual_message_label.setStyleSheet(f"QLabel {{ background: {COLORS['RED_BACKGROUND']}; color: {COLORS['RED']}; padding: 10px; border-radius: 5px; border: 1px solid {COLORS['RED']}; }}")
        self.manual_message_label.setCursor(Qt.CursorShape.ArrowCursor)
    elif msg_type == "success":
        self.manual_message_label.setStyleSheet(f"QLabel {{ background: {COLORS['GREEN_BACKGROUND']}; color: {COLORS['GREEN']}; padding: 10px; border-radius: 5px; border: 1px solid {COLORS['GREEN']}; }}")
    ...
    text_rect = font_metrics.boundingRect(
        0, 0, self.manual_message_label.width() - 22,  # Account for padding (10px * 2) + border (1px * 2)
        0, 
...
def _on_sync_finished(self, success, result_file, message):
    ...
    if success and result_file:
        ...
        if is_actual_overwrite and hasattr(self.manual_input_box, 'total_shifted_ms'):
            ...
            self.manual_input_box.total_shifted_ms = self.session_shifted_files[self.manual_input_box.file_path]
            # Update the display
            self._update_total_shifted_display()
            
            # Log that we tracked an overwrite
            logger.info(f"Tracked shift of {shift_ms}ms for {self.manual_input_box.file_path}, new total: {self.manual_input_box.total_shifted_ms}ms")
...
def _update_total_shifted_display(self):
    ...
    if not hasattr(self, 'manual_input_box') or not hasattr(self.manual_input_box, 'total_shifted_label') or not self.manual_input_box.file_path:
        return
    ...
    if not will_overwrite:
        add_prefix = self.add_ms_prefix_checkbox.isChecked() if hasattr(self, 'add_ms_prefix_checkbox') else True
        if add_prefix:
            ...
            output_file = determine_manual_output_path(self, self.manual_input_box.file_path, 0, add_prefix)
            will_overwrite = os.path.abspath(self.manual_input_box.file_path) == os.path.abspath(output_file)
    ...
    if will_overwrite and hasattr(self, 'session_shifted_files'):
        ...
        if total_ms != 0:
            ...
            self.manual_input_box.total_shifted_label.move(10, self.manual_input_box.height() - self.manual_input_box.total_shifted_label.height() - 10)
            return
...
def _update_prefix_checkbox_state(self):
    ...
    if save_location == "overwrite_input_subtitle":
        ...
        self.add_ms_prefix_checkbox.setToolTip("Prefix is not applicable when overwriting the input file")
    else:
...
def _on_settings_changed(self, value=None):
    ...
    if hasattr(self, 'manual_save_combo') and self.sender() == self.manual_save_combo:
        ...
        self._update_prefix_checkbox_state()
    elif hasattr(self, 'add_ms_prefix_checkbox') and self.sender() == self.add_ms_prefix_checkbox:
...
def _on_message_clicked(self, event):
    ...
    if (hasattr(self.manual_message_label, 'message_type') and 
        self.manual_message_label.message_type == "success" and 
        hasattr(self.manual_message_label, 'result_file_path')):
