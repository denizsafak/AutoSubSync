...
def get_encoding_setting(app):
    return app.config.get("output_subtitle_encoding", DEFAULT_OPTIONS["output_subtitle_encoding"])
...
def match_output_encoding(app, in_path, out_path):
    ...
    if enc == "disabled":
        ...
        match_subtitle_encoding(in_path, out_path, getattr(app, "log_window", None), enc)

def cleanup_files(files, folder=None):
    for f in files:
        try:
            if os.path.exists(f): os.remove(f)
        except OSError as e: logger.error(f"Failed to remove {f}: {e}")
    if folder and os.path.exists(folder):
        try:
            import shutil; shutil.rmtree(folder)
        except OSError as e: logger.error(f"Failed to remove {folder}: {e}")

def append_log(app, msg, color=None, bold=False, end="\n", overwrite=False):
    if hasattr(app, "log_window"): app.log_window.append_message(msg, color=color, bold=bold, end=end, overwrite=overwrite)

def update_progress(app, percent, idx=None, total=None):
    if hasattr(app, "log_window"): app.log_window.update_progress(percent, idx, total)
...
def handle_completion(app, ok, out, in_path):
    ...
    if ok and out and os.path.exists(out):
        try: match_output_encoding(app, in_path, out)
        except Exception as e: logger.warning(f"Failed to match subtitle encoding: {e}")
    return ok
...
def shorten_progress_bar(line):
    start = line.find("["); end = line.find("]", start)
    if start != -1 and end != -1:
        percent = float(line[line.find(" ", end)+1:line.find("%", end)])
        width, filled = 25, int(25*percent/100)
        new_bar = "["+"="*(filled-1)+">"+"-"*(width-filled)+"]" if filled<width else "["+"="*width+"]"
...
class SyncProcess:
    ...
    def run_sync(self, reference, subtitle, tool="ffsubsync", output=None):
        if hasattr(self.app, "log_window"):
            ...
            self.app.log_window.append_message(""); self.app.log_window.cancel_button.setEnabled(True)
    ...
    def _run(self, reference, subtitle, tool, output):
        try:
            if not os.path.exists(reference) and not os.path.exists(subtitle):
                self.signals.error.emit("Skipping: Both files does not exist"); self.signals.finished.emit(False, None); return
            elif not os.path.exists(reference):
                self.signals.error.emit("Skipping: Reference file does not exist"); self.signals.finished.emit(False, None); return
            elif not os.path.exists(subtitle):
                self.signals.error.emit("Skipping: Subtitle file does not exist"); self.signals.finished.emit(False, None); return
            ...
            if tool_type == "module":
                ...
                if not exe: self.signals.error.emit(f"No executable found for {tool} on {current_os}"); self.signals.finished.emit(False, None); return
                if not output: output = determine_output_path(self.app, reference, subtitle)
                ...
                if backup_enabled and os.path.exists(output):
                    try: create_backup(output)
                    except Exception as e: logger.error(f"Failed to create backup: {e}")
            ...
            elif self.should_cancel: self.signals.finished.emit(False, None)
            else: self.signals.finished.emit(True, output)
        except Exception as e:
            if not self.should_cancel:
                ...
                self.signals.error.emit(error_msg); self.signals.finished.emit(False, None)
            else: self.signals.finished.emit(False, None)
    ...
    def _build_cmd(self, tool, exe, reference, subtitle, output):
        ...
        if tool == "alass":
            try:
                ...
                new_subtitle_encoding = subtitle_encoding if subtitle_encoding in enc_list else find_closest_encoding(subtitle_encoding)
                cmd.extend(["--encoding-inc", new_subtitle_encoding])
            except Exception as e: logger.warning(f"Failed to detect subtitle encoding: {e}")
            ref_ext = os.path.splitext(reference)[1].lower()
            if ref_ext in SUBTITLE_EXTENSIONS:
                try:
                    ref_encoding = detect_encoding(reference)
                    new_ref_encoding = ref_encoding if ref_encoding in enc_list else find_closest_encoding(ref_encoding)
                    cmd.extend(["--encoding-ref", new_ref_encoding])
                except Exception as e: logger.warning(f"Failed to detect reference encoding: {e}")
        return self._append_opts(cmd, tool)
    def _append_opts(self, cmd, tool):
        config = self.app.config; info = SYNC_TOOLS.get(tool, {})
        for name, opt in info.get("options", {}).items():
            ...
            if arg and val != default:
                if name == "split_penalty" and val == -1:
                    ...
                    if no_splits_arg: cmd.append(no_splits_arg)
                elif isinstance(default, bool): cmd.append(arg)
                else: cmd.extend([arg, str(val)])
...
class LogWindowStream:
    def __init__(self, emit_func, progress_percent_emit=None, idx=None, total=None):
        self.emit_func, self.progress_percent_emit, self.idx, self.total = emit_func, progress_percent_emit, idx, total
    ...
    def write(self, s):
        ...
        while True:
            ...
            if idx == -1: break
            ch = self._buffer[idx]; line, self._buffer = self._buffer[:idx], self._buffer[idx+1:]
            display_line = line
            if self.idx is not None and self.total is not None:
                percent_match = re.search(r"(\d{1,2}(?:\.\d{1,2})?)\s*%", line)
                if percent_match: display_line = f"{line} [{self.idx}/{self.total}]"
            if self.progress_percent_emit:
                percent_match = re.search(r"(\d{1,2}(?:\.\d{1,2})?)\s*%", line)
                if percent_match:
                    try: self.progress_percent_emit(float(percent_match.group(1)))
                    except Exception: pass
            if ch == "\r": self.emit_func(display_line, True); self._last_was_cr = True
            else: self.emit_func(display_line, self._last_was_cr); self._last_was_cr = False
    def flush(self):
        if self._buffer:
            display_line = self._buffer
            if self.idx is not None and self.total is not None:
                percent_match = re.search(r"(\d{1,2}(?:\.\d{1,2})?)\s*%", self._buffer)
                if percent_match: display_line = f"{self._buffer} [{self.idx}/{self.total}]"
            if self.progress_percent_emit:
                percent_match = re.search(r"(\d{1,2}(?:\.\d{1,2})?)\s*%", self._buffer)
                if percent_match:
                    try: self.progress_percent_emit(float(percent_match.group(1)))
                    except Exception: pass
            self.emit_func(display_line, self._last_was_cr); self._buffer = ""; self._last_was_cr = False

def start_sync_process(app):
    try:
        if hasattr(app, "log_window"): app.log_window.reset_for_new_sync()
        items = (
            [{"reference_path": vp, "subtitle_path": sp} for vp, sp in app.batch_tree_view.get_all_valid_pairs()]
        ...
        current_item_idx, batch_success_count, batch_fail_count, total_items, failed_pairs = 0, 0, 0, len(items), []
        app._batch_state = {"should_cancel": False, "current_process": None}
        if app.batch_mode_enabled and len(items) > 1:
            def cancel_batch():
                from PyQt6.QtWidgets import QMessageBox
                reply = QMessageBox.question(app, "Cancel Batch Sync", f"Are you sure you want to cancel batch sync?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
                if reply == QMessageBox.StandardButton.No: return
                if hasattr(app, "_batch_state"): app._batch_state["should_cancel"] = True; current_proc = app._batch_state.get("current_process");
                if current_proc: current_proc.cancel()
                ...
                else: app.restore_auto_sync_tab()
            app.log_window.cancel_clicked.disconnect(); app.log_window.cancel_clicked.connect(cancel_batch)
        else:
            def cancel_single():
                if hasattr(app, "_current_sync_process"): app._current_sync_process.cancel()
                app.restore_auto_sync_tab()
            app.log_window.cancel_clicked.disconnect(); app.log_window.cancel_clicked.connect(cancel_single)
        def process_next_item():
            ...
            if hasattr(app, "_batch_state") and app._batch_state.get("should_cancel", False):
                ...
                if hasattr(app, "_batch_state"): del app._batch_state
                return
                ...
                if app.batch_mode_enabled and total_items > 1:
                    ...
                    if batch_fail_count > 0:
                        ...
                        for fail_idx, v, s in failed_pairs:
                            ...
                            append_log(app, "Reference: ", end=""); append_log(app, v, COLORS["ORANGE"], end="\n")
                            append_log(app, "Subtitle: ", end=""); append_log(app, s, COLORS["ORANGE"], end="\n\n")
                    app.log_window.finish_batch_sync()
                if hasattr(app, "_batch_state"): del app._batch_state
                return
            it = items[current_item_idx]; original_idx = current_item_idx
            ...
            check_video_subs = is_video_ref and app.config.get(f"{tool}_check_video_for_subtitles", SYNC_TOOLS[tool].get("options", {}).get("check_video_for_subtitles", {}).get("default", False))
            ...
            if check_video_subs:
                ...
                def run_extraction():
                    try:
                        ...
                        with log_lock: extraction_result[2].extend(result[2])
                    except Exception as e:
                        logger.exception(f"Extraction failed: {e}");
                        with log_lock: extraction_result[2].append(f"Extraction failed: {str(e)}")
                    finally: extraction_done.set()
            ...
            def convert_if_needed(file_path):
                ...
                if ext in SUBTITLE_EXTENSIONS and ext not in supported:
                    ...
                    for msg in msgs: append_log(app, msg, COLORS["GREY"])
                    if converted:
            ...
            if not reference_path or not subtitle_path:
                if app.batch_mode_enabled and total_items > 1:
                    batch_fail_count += 1; failed_pairs.append((original_idx, original_ref_path, original_sub_path))
                    ...
                    app.restore_auto_sync_tab()
                return
            proc = SyncProcess(app); app._current_sync_process = proc
            if hasattr(app, "_batch_state"): app._batch_state["current_process"] = proc
            ...
            def batch_completion_handler(ok, out):
                ...
                if ok: batch_success_count += 1; cleanup_files(converted_files_to_clean, extracted_folder_to_clean)
                else: batch_fail_count += 1; failed_pairs.append((original_idx, original_ref_path, original_sub_path))
                update_progress(app, int((original_idx+1)*100/total_items), original_idx+1, total_items)
            ...
            def single_completion_handler(ok, out):
                ...
                if ok: cleanup_files(converted_files_to_clean, extracted_folder_to_clean)
            if app.batch_mode_enabled and len(items) > 1:
...
def determine_output_path(app, reference, subtitle, subtitle_was_converted=False):
    ...
    if subtitle_was_converted: sub_ext = ".srt"
    if save_loc == "save_next_to_input_subtitle":
        ...
        folder = config.get("automatic_save_folder", ""); out_dir = folder if folder and os.path.isdir(folder) else sub_dir; out_name = f"{prefix}{sub_name}{sub_ext}"
    else:
        out_dir, out_name = sub_dir, f"{prefix}{sub_name}{sub_ext}"
    output_path = os.path.join(out_dir, out_name)
    if save_loc not in ("save_next_to_video_with_same_filename", "overwrite_input_subtitle"):
        base, ext = os.path.splitext(out_name); counter = 2
        while os.path.exists(output_path):
            out_name = f"{base}_{counter}{ext}"; output_path = os.path.join(out_dir, out_name); counter += 1
    return output_path
